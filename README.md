...
<!-- SELECT * FROM `emploiyees` ORDER BY `first_name` DESC -->
...
<!-- SELECT SUM(`salary`) AS Total_salaries_pay FROM `employees` -->
...
<!-- SELECT SUBSTR(`first_name`, 1, 3) AS Troi_pre_cara_Prenom From `employees` -->
...
<!-- SELECT `first_name`, `last_name`, CAST( DATEDIFF ( job_history.end_date, job_history.start_date)/365 AS SIGNED INTEGER ) AS nember_anneé_taravail FROM `job_history`, `employees` WHERE job_history.employee_id = employees.employee_id -->
